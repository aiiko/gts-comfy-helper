...
func TestBuildCharacterDefinitionWithGiantessName(t *testing.T) {
	got, err := buildCharacterDefinition(1, "Alice", "destroying buildings", "count", 2, "", "", "climbing her leg")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	want := "1girl, Alice, a giantess girl destroying buildings, 2 tinies climbing her leg"
	if got != want {
		t.Fatalf("character definition mismatch:\nwant: %q\ngot:  %q", want, got)
	}
}

func TestBuildCharacterDefinitionWithGiantessNameForTwo(t *testing.T) {
	got, err := buildCharacterDefinition(2, "Alice", "destroying buildings", "count", 2, "", "", "climbing her leg")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	want := "2girls, Alice, two giantess girls destroying buildings, 2 tinies climbing her leg"
	if got != want {
		t.Fatalf("character definition mismatch:\nwant: %q\ngot:  %q", want, got)
	}
}
